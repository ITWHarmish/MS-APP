"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteToDo = exports.updateToDo = exports.createToDo = exports.getToDoList = void 0;
const todo_1 = __importDefault(require("../models/todo"));
const getToDoList = (uid) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield todo_1.default.find({ userId: uid });
    return result;
});
exports.getToDoList = getToDoList;
const createToDo = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const todoData = new todo_1.default(payload);
    yield todoData.save();
    return todoData;
});
exports.createToDo = createToDo;
const updateToDo = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    yield todo_1.default.findByIdAndUpdate(id, payload, { new: true });
});
exports.updateToDo = updateToDo;
const deleteToDo = (id) => __awaiter(void 0, void 0, void 0, function* () {
    yield todo_1.default.findByIdAndRemove(id);
});
exports.deleteToDo = deleteToDo;
